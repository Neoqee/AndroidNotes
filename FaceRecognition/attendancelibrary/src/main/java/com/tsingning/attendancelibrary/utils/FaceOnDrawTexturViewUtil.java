package com.tsingning.attendancelibrary.utils;
...
public class FaceOnDrawTexturViewUtil {
    ...
    public static Rect getFaceRectTwo(Context context,FaceInfo faceInfo) {
        Rect rect = new Rect();
        float offset = 0;
        if (SingleBaseConfig.getBaseConfig().getMirrorRGB() == 1){
            int width = DensityUtils.getDisplayWidth(context) / 2;
            offset = (width - faceInfo.centerX) * 2;
            Log.e("Neoqee","width = " + width + "centerX = " + faceInfo.centerX + "offset = " + offset);
        }
        rect.top = (int) ((faceInfo.centerY - faceInfo.width / 1.3));
        rect.left = (int) ((faceInfo.centerX - faceInfo.width / 2) + offset);
        rect.right = (int) ((faceInfo.centerX + faceInfo.width / 2) + offset);
        rect.bottom = (int) ((faceInfo.centerY + faceInfo.width / 1.8));
        return rect;
    }
