package com.tsingning.registerlibrary.config;
...
public class BaseConfig {
    ...
    // NIR或depth实时视频预览
    private Boolean isNirOrDepth = false;
    ...
    private int videoDirection = 90;     // 有90°的旋转
...
//    private int detectDirection = 270;    // oppo设备有270°的旋转
    private int detectDirection = 90;    // 设备有90°的旋转
    ...
    private int liveThreshold = 75;
    // 识别阈值，0-100，默认为80分,需要选择具体模型的阈值。live：80、idcard：80
    private int idThreshold = 75;
    // 识别阈值，0-100，默认为80分,需要选择具体模型的阈值。live：80、idcard：80
    private int rgbAndNirThreshold = 75;
    // 模态切换光线阈值
    private int camera_lightThreshold = 50;
    // 使用的特征抽取模型默认为生活照：1；证件照：2；RGB+NIR混合模态模型：3；
    private int activeModel = 1;
    // 识别结果出来后的演示展示，默认为0ms
    private int timeLapse = 60;
    // RGB活体：1
    // RGB+NIR活体：2
    // RGB+Depth活体：3
    // RGB+NIR+Depth活体：4
    //    private int type = 1;
    private int type = 0;   // 无活体检测
    // 是否开启质量检测开关
    ...
    // 0：RGB无镜像，1：有镜像
    private int mirrorRGB = 0;    // 设备无
//    private int mirrorRGB = 1;      // oppo有镜像
    // 0：NIR无镜像，1：有镜像
